...
setup_nameservers() {
  ...
  NS1=$(terraform show --json | jq --raw-output '.values.root_module.resources[] | select(.address == "aws_route53_zone.diff_data_com_r53_zone") | .values.name_servers[0]')
  NS2=$(terraform show --json | jq --raw-output '.values.root_module.resources[] | select(.address == "aws_route53_zone.diff_data_com_r53_zone") | .values.name_servers[1]')
  NS3=$(terraform show --json | jq --raw-output '.values.root_module.resources[] | select(.address == "aws_route53_zone.diff_data_com_r53_zone") | .values.name_servers[2]')
  NS4=$(terraform show --json | jq --raw-output '.values.root_module.resources[] | select(.address == "aws_route53_zone.diff_data_com_r53_zone") | .values.name_servers[3]')
